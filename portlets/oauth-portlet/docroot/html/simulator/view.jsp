...
<c:choose>
	<c:when test="<%= verifyStep %>">
		<liferay-ui:panel title="Authorize Application?">
			<span>
				You decided to authorize this application to access your resources? To complete process click the
				<aui:a href="<%= oauthURL %>">LINK</aui:a>.
			</span>
		</liferay-ui:panel>
	...
	<c:otherwise>
		...
		>
			...
			<%
			List<OAuthApplication> oAuthApps = OAuthApplicationLocalServiceUtil.getApplications(themeDisplay.getCompanyId());
			int oAuthAppsCnt = OAuthApplicationLocalServiceUtil.getApplicationsCount(themeDisplay.getCompanyId());
			%>
